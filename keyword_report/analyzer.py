...
def extract_business_info(pages: list[ScrapedPage]) -> BusinessProfile:
    ...
    response = client.messages.create(
        ...
        system=(
            ...
            "cities/suburbs. IMPORTANT: Always include the nearest major metro or large city (e.g. if the business "
            "is in Branford CT, include 'New Haven'; if in Coral Springs FL, include 'Fort Lauderdale'; if in "
            "Setauket NY, include 'Long Island'). This ensures we find keyword data for the broader area. "
            "Return at least 8 cities. For non-local businesses, return empty list.\n\n"
            ...
            "RELEVANCE_TERMS: 20-30 SHORT words (1-2 words max) that signal a keyword is relevant to this business. "
            "These are used as substring filters against raw keyword data from Google, so they MUST be simple root words "
            "that appear inside keywords. Good examples: 'plumber', 'plumbing', 'heating', 'drain', 'roof', 'paint', "
            "'pool', 'dental', 'lawyer'. BAD examples (too specific/long): 'licensed plumber', 'certified heating technician', "
